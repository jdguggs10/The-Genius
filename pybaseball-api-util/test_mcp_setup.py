...
def test_startup_scripts():
    ...
    # Also check that the server files exist
    server_files = [
        "/Users/geraldgugger/Code/the-genius/pybaseball-api-util/pybaseball_mcp_server_v2.py"
    ]
    ...
    for server_file in server_files:
        if os.path.exists(server_file):
            print(f"✅ {os.path.basename(server_file)} exists")
        else:
            print(f"❌ {os.path.basename(server_file)} missing")
            all_good = False
